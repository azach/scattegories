...
- `#g=<seed>&t=<startMs>` — added when someone presses **Start**. The countdown is computed from this
  absolute timestamp, so every open link shows the same time remaining (relies on devices' clocks being
  roughly in sync, which they normally are via NTP).
...
3. Press **Start the 3:00 timer**, then share the link again for a synced clock.
